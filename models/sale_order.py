...
"")
    ...
    def _recompute_attachment_url(self,numero_contratto=None):
        for record in self:
            attachment = self.env['ir.attachment'].search([('res_model', '=', 'res.partner'), ('res_id', '=', 1)], limit=1)
            new_attach=attachment.add_text_and_save_to_partner(record.id, 'Contratto n. %s ' % numero_contratto if numero_contratto else record.numero_contratto, x=10, y=900)            # Uso del metodo
